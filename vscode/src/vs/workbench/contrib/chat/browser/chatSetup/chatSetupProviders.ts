...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatModeKind, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, 'CodeCrab Assistant', true, description, location, mode, context, controller);
		});
	}
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpCopilotNeeded', "CodeCrab AI Router is offline. Please start it on port 3141 to use Chat."));
	...
	async invoke(request: IChatAgentRequest, progress: (parts: IChatProgress[]) => void): Promise<IChatAgentResult> {
		return this.instantiationService.invokeFunction(async accessor => {
			...
			return this.doInvoke(request, part => progress([part]), languageModelsService, editorService, workspaceService);
		});
	}

	private async doInvoke(request: IChatAgentRequest, progress: (part: IChatProgress) => void, languageModelsService: ILanguageModelsService, editorService: IEditorService, workspaceService: IWorkspaceContextService): Promise<IChatAgentResult> {
		try {
			const messageContent = request.message;

			// Gather workspace root path
			...
			// Gather open file paths from visible editors
			const openFiles: string[] = [];
			try {
				const visibleControls = editorService.visibleTextEditorControls;
			...
			console.log(`[CodeCrab] workspaceRoot=${workspaceRoot}, openFiles=${openFiles.length}, msg=${messageContent.length} chars`);

			const chatMessage = {
				role: 1, /* ChatMessageRole.User */
				content: [{ type: 'text', value: messageContent }]
			};
			const models = await languageModelsService.selectLanguageModels({ id: 'codecrab/auto' });
			if (models.length === 0) {
				throw new Error('Chat provider for model codecrab/auto is not registered.');
			}

			// Pass workspace info through options — the model provider extracts them
			const options: any = { workspaceRoot, openFiles };

			const response = await languageModelsService.sendChatRequest(models[0], undefined, [chatMessage as any], options, CancellationToken.None);

			// Stream the response directly to the UI so it doesn't get stuck on "Evaluating"
			for await (const chunk of response.stream) {
				const text = (chunk as any).value ?? '';
				if (text) {
					progress({ kind: 'markdownContent', content: new MarkdownString(text) });
				}
			}
		} catch (err: any) {
			progress({ kind: 'markdownContent', content: new MarkdownString(`**Error:** ${err.message || err}\n\nStack: ${err.stack || ''}`) });
		}
		return {};
	}
